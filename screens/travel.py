...
class Travel(MDScreen):
    ...
    def generate(self):
        ...
        def findRoute(rute, antNumber, temp_visibilty, pheromone):
            ...
            for j in range(n - 1):  # untuk setiap bandara yang ada
                if sampai is False:
                    ...
                    city = np.nonzero(cum_prob > r)[0][0]  # mencari rute selanjutnya dengan kemungkinan bilangan lebih besar dari r yang pertama
                    rute[i, j + 1] = city
                    if city == destinasi:
                        sampai = True
                else:
        ...
        def totalDistanceTour(tour):  # pada rute tempuh sebuah semut
            jarak = 0  # inisialisasi awal 0
            # print("Tour : ", tour)
            ...
            for i in range(0, len(tour) - 1):  # panjang tour -1 0-6 jika bandara = 7
                jarak = jarak + d[tour[i]][tour[i + 1]]  # jarak dari kota awal menuju kota tujuan
            return jarak
